...
def main():
    ...
    for band in ("gband", "yband", "zband"):
        ...
        for length in ("10mm", "30mm", "50mm"):
            ...
            x = np.array(list(sorted_angle_distribution.keys()))
            y = np.array(list(sorted_angle_distribution.values()))
            axs.plot(
                x,
                y * 10 ** 3,
                color + 'o',
        ...
        plt.savefig("/tmp/" + f"{band}_ad_electrode_length" + ".pdf", format="pdf")
        plt.show()
        plt.close()
    return
